...
export async function createWorkspace(input) {
    ...
    const { data: newWorkspace, error: workspaceError } = await supabase
        ...
        .single();
    if (workspaceError) {
        throw new DatabaseError(`Failed to create workspace: ${workspaceError.message}`, workspaceError);
    }
    if (!newWorkspace) {
    ...
    if (input.userId) {
        ...
        if (memberError) {
            throw new DatabaseError(`Failed to add workspace member: ${memberError.message}`, memberError);
        }
    }
    ...
    const template = await supabase
        ...
        .maybeSingle();
    if (template.error) {
        throw new DatabaseError(`Failed to fetch template workspace: ${template.error.message}`, template.error);
    }
    if (!template.data) {
        throw new NotFoundError('Template workspace not found or deleted.', 'workspace');
    }
    ...
    if (sources && sources.length > 0) {
        for (const row of sources) {
            ...
            if (inserted && inserted.id) {
                sourceIdMap.set(row.id, inserted.id);
            }
        }
    }
    ...
    if (events && events.length > 0) {
        for (const row of events) {
            ...
            const { data: inserted } = await supabase
                ...
                .single();
            if (inserted && inserted.id) {
                eventIdMap.set(row.id, inserted.id);
            }
        }
    }
    ...
    if (settings) {
        await supabase.from('workspace_settings').insert({
            workspace_id: newWorkspaceId,
            audit_rules_json: settings.audit_rules_json ?? '{}',
...
async function ensureWorkspaceSettings(supabase, workspaceId, opts) {
    ...
    if (error) {
        throw new DatabaseError(`Failed to create workspace settings: ${error.message}`, error);
    }
}
