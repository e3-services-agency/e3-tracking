 ...
 */
export async function validatePayload(workspaceId, eventId, actualJson) {
 ...
 */
router.get('/:id/export/html', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({
            error: 'Workspace context required.',
 ...
 */
router.post('/:id/share', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({
            error: 'Workspace context required.',
 ...
 */
router.put('/:id/canvas', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({
            error: 'Workspace context required.',
    ...
    try {
        const updated = await JourneyDAL.saveJourneyCanvas(workspaceId, journeyId, nodes ?? [], edges ?? []);
...
router.patch('/:id', requireWorkspace, async (req, res) => {
    ...
    const journeyId = req.params.id;
    const body = req.body;
    const testing_instructions_markdown = typeof body.testing_instructions_markdown === 'string'
 ...
 */
router.post('/:id/events/:eventId/qa/validate', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({
            error: 'Workspace context required.',
