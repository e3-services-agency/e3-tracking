 ...
 */
router.get('/:id/fields', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({ error: 'Workspace context required.', code: 'WORKSPACE_REQUIRED' });
        return;
 ...
 */
router.post('/:id/fields', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({ error: 'Workspace context required.', code: 'WORKSPACE_REQUIRED' });
        return;
...
router.patch('/:id', requireWorkspace, async (req, res) => {
    ...
    const body = req.body;
    try {
        const catalogTypePatch = body.catalog_type === 'Product' || body.catalog_type === 'Variant' || body.catalog_type === 'General'
            ? body.catalog_type
            : undefined;
 ...
 */
router.patch('/:catalogId/fields/:fieldId', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({ error: 'Workspace context required.', code: 'WORKSPACE_REQUIRED' });
        return;
    ...
    try {
        if (setLookupKey) {
            const field = await CatalogService.setCatalogFieldLookupKey(workspaceId, catalogId, fieldId);
            res.status(200).json(field);
            return;
        }
        const field = await CatalogService.updateCatalogField(workspaceId, catalogId, fieldId, {
            name: typeof body.name === 'string' ? body.name : undefined,
 ...
 */
router.delete('/:catalogId/fields/:fieldId', requireWorkspace, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({ error: 'Workspace context required.', code: 'WORKSPACE_REQUIRED' });
        return;
