...
const VALID_CATALOG_TYPES = ['Product', 'Variant', 'General'];
function normalizeCatalogType(v) {
    return VALID_CATALOG_TYPES.includes(v) ? v : 'General';
}
...
export async function getCatalogById(workspaceId, catalogId) {
    const supabase = getSupabase();
    const { data, error } = await supabase
        .from('catalogs')
        .select('*')
...
export async function updateCatalog(workspaceId, catalogId, input) {
    ...
    if (input.source_system !== undefined)
        updates.source_system = input.source_system.trim();
    if (input.sync_method !== undefined)
        updates.sync_method = input.sync_method.trim();
    if (input.update_frequency !== undefined)
        updates.update_frequency = input.update_frequency.trim();
    if (input.catalog_type !== undefined)
        updates.catalog_type = normalizeCatalogType(input.catalog_type);
    const { data, error } = await supabase
        .from('catalogs')
        .update(updates)
        .eq('id', catalogId)
...
// ----- Catalog Fields -----
export async function listCatalogFields(catalogId) {
    const supabase = getSupabase();
    const { data, error } = await supabase
        .from('catalog_fields')
        .select('*')
...
export async function listCatalogFieldsForWorkspace(workspaceId, catalogId) {
    await getCatalogOrThrow(workspaceId, catalogId);
...
export async function updateCatalogField(workspaceId, catalogId, fieldId, input) {
    ...
    if (input.is_lookup_key !== undefined)
        updates.is_lookup_key = Boolean(input.is_lookup_key);
    const { data, error } = await supabase
        .from('catalog_fields')
        .update(updates)
        .eq('id', fieldId)
