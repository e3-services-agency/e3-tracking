...
function buildPayloadDoc(
  /** Rows may already include variant merges from `mergeEventPropertyWithDetailsWithEffectiveList`. */
...
export async function generateJourneyHtmlExport(
  ...
  // Simple flat sidebar — one anchor per step, no tabs, no QA pane. The QA
  // overlay injection script renders QA-run sections inline at the top of
  // `.export-main` instead of populating a separate sidebar pane.
  const tocHtml =
    steps.length > 0
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .export-step-meta-value--code {
      ...
      border: 1px solid #e5e7eb;
      border-radius: 6px;
      /* Child .export-meta-target-code already wraps via white-space:pre-wrap +
         word-break:break-all, so the wrapper never needs to scroll. Keep
         hidden as a defensive guarantee (matches every other code block in
         the export). */
      overflow-x: hidden;
    }
