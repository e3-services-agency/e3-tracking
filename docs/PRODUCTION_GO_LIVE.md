...
- **Build:** The API handler **`api/[[...path]].ts`** imports the bundled backend from **`api/_backend/app.js`** (generated by **`tsup`**). **Build Command** in Vercel: **`npm run build`** (runs `build:backend` then Vite). **Output Directory**: **`dist`**.
