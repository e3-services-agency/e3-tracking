...
const router = Router();
const CONTEXTS = ['event_property', 'user_property', 'system_property'];
const DATA_TYPES = ['string', 'integer', 'float', 'boolean', 'object', 'list'];
const PII_STATUSES = ['none', 'sensitive', 'highly_sensitive'];
 ...
 */
router.post('/', requireWorkspace, propertyAuditValidator, async (req, res) => {
    const workspaceId = req.workspaceId;
    if (!workspaceId) {
        res.status(403).json({
    ...
    const piiStatus = body.pii_status ?? 'none';
    if (!PII_STATUSES.includes(piiStatus)) {
        res.status(400).json({
    ...
    const propertyData = {
        ...
        mapping_type: body.mapping_type === 'lookup_key' || body.mapping_type === 'mapped_value' ? body.mapping_type : undefined,
    };
    try {
...
router.patch('/:id', requireWorkspace, async (req, res) => {
    ...
    if (body.pii_status !== undefined && PII_STATUSES.includes(body.pii_status)) {
        updates.pii_status = body.pii_status;
    }
    if (body.data_type !== undefined && DATA_TYPES.includes(body.data_type)) {
        updates.data_type = body.data_type;
    }
    if (body.data_format !== undefined)
        updates.data_format = body.data_format === null ? null : String(body.data_format);
    if (typeof body.is_list === 'boolean')
        updates.is_list = body.is_list;
    if (body.example_values_json !== undefined)
        updates.example_values_json = body.example_values_json;
    if (body.name_mappings_json !== undefined)
        updates.name_mappings_json = body.name_mappings_json;
