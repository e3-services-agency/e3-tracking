...
function computeTypeCounts(nodes) {
    const counts = { new: 0, enrichment: 0, fix: 0 };
    if (!Array.isArray(nodes))
        return counts;
    for (const node of nodes) {
        if (node?.type !== 'journeyStepNode')
            continue;
        const t = node?.data?.implementationType;
        if (t === 'new' || t === 'enrichment' || t === 'fix')
            counts[t] += 1;
    }
    return counts;
}
 ...
 */
export async function getJourneyById(workspaceId, journeyId) {
 ...
 */
export async function listJourneys(workspaceId) {
    const supabase = getSupabase();
    const { data, error } = await supabase
        .from('journeys')
        .select('*')
 ...
 */
function getEventIdsFromNodes(nodes) {
    if (!Array.isArray(nodes))
        return [];
    const ids = [];
    for (const node of nodes) {
 ...
 */
export async function saveJourneyCanvas(workspaceId, journeyId, nodes, edges) {
    ...
    const { error: updateError } = await supabase
        ...
        .eq('workspace_id', workspaceId);
    if (updateError) {
        throw new DatabaseError(`Failed to save journey canvas: ${updateError.message}`, updateError);
    }
    ...
    if (deleteError) {
        throw new DatabaseError(`Failed to sync journey_events: ${deleteError.message}`, deleteError);
    }
    if (eventIds.length > 0) {
        ...
        const { error: insertError } = await supabase
            ...
            .insert(rows);
        if (insertError) {
            throw new DatabaseError(`Failed to insert journey_events: ${insertError.message}`, insertError);
        }
    }
 ...
 * @throws NotFoundError when journey is not in workspace.
 */
export async function updateJourney(workspaceId, journeyId, patch) {
    const journey = await getJourneyById(workspaceId, journeyId);
    if (journey === null) {
        throw new NotFoundError('Journey not found or does not belong to this workspace.', 'journey');
    }
    ...
    const { error } = await supabase
        ...
        .eq('workspace_id', workspaceId);
    if (error) {
        throw new DatabaseError(`Failed to update journey: ${error.message}`, error);
    }
    const updated = await getJourneyById(workspaceId, journeyId);
    if (updated === null) {
 ...
 * @throws NotFoundError when journey is not in workspace.
 */
export async function generateShareToken(workspaceId, journeyId) {
    const journey = await getJourneyById(workspaceId, journeyId);
    if (journey === null) {
        throw new NotFoundError('Journey not found or does not belong to this workspace.', 'journey');
    }
    ...
    const { error } = await supabase
        ...
        .eq('workspace_id', workspaceId);
    if (error) {
        throw new DatabaseError(`Failed to generate share token: ${error.message}`, error);
    }
    return token;
}
/**
 ...
 */
export async function getJourneyByShareToken(token) {
    const supabase = getSupabase();
    const { data, error } = await supabase
        .from('journeys')
        ...
        .maybeSingle();
    if (error) {
        throw new DatabaseError(`Failed to fetch journey by share token: ${error.message}`, error);
    }
    if (data === null) {
