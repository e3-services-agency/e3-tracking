 ...
 */
export function buildCodegenSnippets(eventName, attached) {
    const safeName = eventName?.trim() || 'event_name';
    const props = attached.filter((p) => p.presence === 'always_sent' || p.presence === 'sometimes_sent');
 ...
 */
export function buildCodegenSnippetsFromPresence(eventName, alwaysSent, sometimesSent) {
    const attached = [
        ...
        ...sometimesSent.map((property_name) => ({
            property_name,
            presence: 'sometimes_sent',
        })),
    ];
    return buildCodegenSnippets(eventName, attached);
}
function formatPropLine(name, optional) {
    const line = `  ${name}: ${PLACEHOLDER}`;
    if (optional)
        return { comment: '  // Optional:', line };
    return { line };
}
function buildDataLayerSnippet(eventName, props) {
    const lines = ['window.dataLayer.push({', `  event: '${eventName}',`];
    for (const p of props) {
        const { comment, line } = formatPropLine(p.property_name, p.presence === 'sometimes_sent');
        if (comment)
            lines.push(comment);
        lines.push(line + ',');
    }
...
function buildBloomreachSdkSnippet(eventName, props) {
    const lines = [`exponea.track('${eventName}', {`];
    for (const p of props) {
        const { comment, line } = formatPropLine(p.property_name, p.presence === 'sometimes_sent');
        if (comment)
            lines.push(comment);
 ...
 */
function buildBloomreachApiSnippet(eventName, props) {
    const properties = {};
    for (const p of props) {
    ...
    const curlExample = `curl -X POST "https://api.exponea.com/track/v2/projects/YOUR_PROJECT_TOKEN/customers/events" \\\n` +
