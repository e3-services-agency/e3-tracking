 ...
 */
export async function createProperty(workspaceId, propertyData) {
    const supabase = getSupabase();
    const row = {
    ...
    if (propertyData.mapping_type !== undefined) {
        row.mapping_type = propertyData.mapping_type ?? null;
    }
    const { data, error } = await supabase
        .from('properties')
        .insert(row)
    ...
    if (error) {
        if (error.code === UNIQUE_VIOLATION_CODE) {
            throw new ConflictError(`A property with the same name and context already exists in this workspace.`, `name="${row.name}", context="${row.context}"`);
        }
        throw new DatabaseError(`Failed to create property: ${error.message}`, error);
    }
    if (data === null) {
 ...
 */
export async function updateProperty(workspaceId, propertyId, updates) {
    const supabase = getSupabase();
    ...
    if (updates.pii_status !== undefined)
        patch.pii_status = updates.pii_status;
    if (updates.data_type !== undefined)
        patch.data_type = updates.data_type;
    if (updates.data_format !== undefined)
        patch.data_format = updates.data_format?.trim() ?? null;
    if (updates.is_list !== undefined)
        patch.is_list = updates.is_list;
    ...
    if (updates.mapped_catalog_id !== undefined)
        patch.mapped_catalog_id = updates.mapped_catalog_id ?? null;
